...
class CoherentGenesisLaserFunctionality(amplitudeModule.AmplitudeFunctionalityBuffered):
    ...
    def output(self, state):
        ...
        fake_arg = 0
        if state:
            subprocess.run([sys.executable, "C:/Users/Light-saver/Documents/storm_control/Hal2/storm_control/sc_hardware/coherent/turnOnCoherent1.py"])

            # self.genesis_laser.turnOn(fake_arg)
            # self.maybeRun(task=self.genesis_laser.turnOn, args=[fake_arg])
        else:
            subprocess.run([sys.executable, "C:/Users/Light-saver/Documents/storm_control/Hal2/storm_control/sc_hardware/coherent/turnOffCoherent1.py"])
...
class CoherentGenesisModule1(amplitudeModule.AmplitudeModule):
