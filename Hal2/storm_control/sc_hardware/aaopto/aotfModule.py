#!/usr/bin/env python
"""
HAL interface to AA Optoelectronics Technologies AOTFs.
...
import storm_control.sc_hardware.aaopto.AOTF as AOTF
...
class AOTFFunctionality(amplitudeModule.AmplitudeFunctionalityBuffered):
    ...
    def onOff(self, power, state):
        self.mustRun(task = self.aotf.channelOnOff,
                     args = [self.channel, state])
        self.on = state

    def output(self, power):
        if self.on:
            print('aotf functionality aa set amplitude')
